...
export class Order {
  ...
  static fromJson(data): Order {
    ...
    o['cancelledBy'] = cancelledByList[data['cancelled_by']];
    ...
    if (data['delivery_eta_mins_when_order_was_accepted_by_vendor']) {
      ...
      if(adminInv['vendor_section']) {
        if(adminInv['vendor_section']['payout_amount_line_items']) {
          for(const i of adminInv['vendor_section']['payout_amount_line_items']) {
            o['adminInvoice']['vendorInvoiceSection']['payoutAmountLineItem'].push(DisplayInvoiceBreakout.fromJson(i));
          }
        }
    ...
    if(adminInv) {
      if(adminInv['customer_order_section']) {
        if(adminInv['customer_order_section']['total_payable_amount']) {
          o['adminInvoice']['customerOrderSection']['totalPayableAmount'] = DisplayInvoiceBreakout.fromJson(adminInv['customer_order_section']['total_payable_amount']);
        }
      }        
        if(adminInv['customer_order_section']) {
          for(const i of adminInv['customer_order_section']['payable_amount_line_items']) {
            o['adminInvoice']['customerOrderSection']['payableAmountLineItem'].push(DisplayInvoiceBreakout.fromJson(i));
          }
        }
        if(adminInv['customer_payout_section']) {
          if(adminInv['customer_payout_section']['total_payout_amount']) {
            o['adminInvoice']['customerPayoutSection']['totalPayoutAmount'] = DisplayInvoiceBreakout.fromJson(adminInv['customer_payout_section']['total_payout_amount']);
          }
        }
        if(adminInv['customer_payout_section']) {
            for(const i of adminInv['customer_payout_section']['payout_amount_line_items']) {
              o['adminInvoice']['customerPayoutSection']['payoutAmountLineItem'].push(DisplayInvoiceBreakout.fromJson(i));
          }
      }
    }
    o['isSponsoredRider'] = data['is_sponsored_rider'];
    ...
    if ( service === 'pnd') {
      ...
      for (const i of data['order_locations']) {
        ...
        location['deliveryOrderLocationId'] = i['delivery_order_location_id'];
...
export class DeliveryOrderStatusHistory {
  ...
  deliveryOrderLocationId: number;
  ...
  static fromJson(data: any): DeliveryOrderStatusHistory {
    ...
    d['status'] = DeliveryStatusList[data['order_delivery_status']];
    d['time'] = data['created_at'];
    d['deliveryRiderId'] = data['rider_id'];
    d['orderLocationDeliveryStatus'] = data['order_location_delivery_status'];
    d['orderLocationId'] = data['order_location_id'];
    d['deliveryOrderLocationId'] = data['delivery_order_location_id'];
    d['orderLocationType'] = data['order_location_type'];
    return d;
  }
...
export class Invoice {
  ...
  static fromJson(data: any): Invoice {
    ...
    if(data['vendor_payout_amount'])
    i['vendorPayoutAmount'] = formatNum(data['vendor_payout_amount']);
    if(data['customer_payout_amount'])
      i['vendorPayoutAmount'] = formatNum(data['customer_payout_amount']);
...
export class AdminInvoice {
  ...
  customerOrderSection: CustomerOrderSection = new CustomerOrderSection();
  customerPayoutSection: CustomerPayoutSection = new CustomerPayoutSection();
}
...
export class VendorInvoiceSection {
  payoutAmountLineItem: DisplayInvoiceBreakout[] = [];
  totalPayoutAmount: DisplayInvoiceBreakout = new DisplayInvoiceBreakout();
}

export class CustomerOrderSection {
  payableAmountLineItem: DisplayInvoiceBreakout[] = [];
  totalPayableAmount: DisplayInvoiceBreakout = new DisplayInvoiceBreakout();
}

export class CustomerPayoutSection {
  payoutAmountLineItem: DisplayInvoiceBreakout[] = [];
  totalPayoutAmount: DisplayInvoiceBreakout = new DisplayInvoiceBreakout();
}
...
export class FilterOrder {
  ...
  toJson(service: string) {
    ...
    if (this.payoutId)
      data['filter']['  tion_id'] = [this.payoutId];
...
export const pndDeliveryStatuesList: { [key in DeliveryStatus]?: string } ={
  pending: 'Pending',
  accepted: 'Accepted',
  rejected: 'Rejected',
  allocated: 'Allocated',
...
export type CancelledBy = 'vendor' | 'customer' | 'delivery_service' | 'admin';
export const cancelledByList: { [key in CancelledBy]: string } = {
  vendor: 'Vendor',
  customer: 'Customer',
