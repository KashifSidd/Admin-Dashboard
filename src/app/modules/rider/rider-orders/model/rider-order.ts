...
export class RiderOrder {
    ...
    orderLocation: OrderLocationDetails [] = [];
    totalClientPodCollectedByRider: string;
    ...
    static fromJson(data): RiderOrder {
        ...
        r['totalClientPodCollectedByRider'] = formatNum(data['total_client_pod_collected_by_rider']);
    ...
    if(data['order_locations']){
        for(const i of data['order_locations']) {
            r['orderLocation'].push(OrderLocationDetails.fromJson(i));
        }
    }
        return r;
    }
...
export class StatusHistory {
    ...
    static fromJson(data): StatusHistory {
        ...
        s['orderLocationDeliveryStatus'] = OrderLocationDeliveryStatus[data['order_location_delivery_status']];
        s['orderLocationId'] = data['order_location_id'];
        s['orderLocationType'] = data['order_location_type'];
...
export type DeliveryStatus = 'PENDING' | 'ASSIGNED' | 'ALLOTTED' | 'ARRIVED' | 'DISPATCHED' | 'ARRIVED_CUSTOMER_DOORSTEP' | 'DELIVERED' | 'CANCELLED' | 'IN_TRANSIT';
...
export enum OrderLocationDeliveryStatus {
    PENDING = 'Pending',
...
export class OrderLocationDetails {
    id: number;
    type: string;
    contactName: string;
    ...
    static fromJson(data): OrderLocationDetails {
        const r: OrderLocationDetails = new OrderLocationDetails();

        r['id'] = data['id'];
        r['type'] = data['type'];
        r['contactName'] = data['contact_name'];
        r['contactNumber'] = data['contact_phone'];
        r['address'] = data['address'];
        r['lat'] = data['coordinates']['latitude'];
        r['long'] = data['coordinates']['longitude'];
        r['clientPodAmount'] = formatNum(data['client_pod_amount']);
        r['arrivalEta'] = data['arrival_eta'];            
        r['visitSequence'] = data['visit_sequence'];            
        r['deliveryCompletedAt'] = data['delivery_completed_at'];
        r['deliveryStatus'] = OrderLocationDeliveryStatus[data['delivery_status']];
        r['podStatus'] = data['pod_status'];
        r['cancellationReason'] = data['cancellation_reason'];
