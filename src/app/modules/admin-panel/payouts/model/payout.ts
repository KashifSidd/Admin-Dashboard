...
export class Payout {
    ...
    static fromJson(data: any): Payout {
        ...
        if(data['amount_paid_to_vendor'])
        p['amountPaidToVendor'] = formatNum(data['amount_paid_to_vendor']);
        if(data['amount_paid_to_merchant'])
        p['amountPaidToVendor'] = formatNum(data['amount_paid_to_merchant']);
        ...
        p['isDeleted'] = data['is_deleted'];
        if(data['payout_details']['account'])
        p['accountDetails'] = PayoutAccount.fromJson(data['payout_details']['account']);
        if(data['payout_details']['merchant'])
        p['accountDetails'] = PayoutAccount.fromJson(data['payout_details']['merchant']);
        p['restaurantId'] = data['payout_details']['restaurant']?.['id'] || data['payout_details']['store']?.['id'] || data['payout_details']['outlet']?.['id'] || data['payout_details']['merchant']['id'];
...
export class FilterPayout {
    ...
    toJson(service: string) {
        ...
        if (this.outletIds) {
            ...
            }else if(service === Services.PND){
                data['merchant_ids'] = this.outletIds.split(',');
            }
        }
        data['filter'] = {}
