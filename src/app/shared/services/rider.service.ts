 ...
 */
  ...
  */
  ...
  /**
 * Method that Sends a POST request to update the rider order with the specified orderId and data.
 */
  postUpdateRiderOrder(orderId: any, data: any): Observable<any> {
    return this.http.post(apiUrls.postAcceptRiderOrderEndPoint(orderId, apiEndPoints[this.service]), data).pipe(
      map((response) => {
        return response;
      })
    );
  ...
  /**
 * Method that sends a PUT request to update the rider's order location with the specified order ID and data.
 */
  putRiderLocationUpdate(orderId: any, data:any): Observable<any> {
    return this.http.put(apiUrls.putRiderOrderLocationEndPoint(orderId, apiEndPoints[this.service]), data).pipe(
      map((response) => {
        return response;
      })
    )
